#!/usr/bin/env python
"""
STEP 1 -- Validation (null test).

Both hemispheres share the SAME (fiducial) cosmology. We build the null
covariance from simulations and check that a full-sky LambdaCDM fit recovers
the input cosmology without bias, and that the chi^2 statistics are sane. This
is the prerequisite before trusting any asymmetric-sky bias.

Run with the env that has healpy+camb+pymaster+iminuit, e.g.:
    /home/ftoscano/anaconda3/envs/spyder/bin/python scripts/run_validation.py --nside 512 --nsims 300
"""
import os
import sys
import argparse
...
def main(args):
    ...
    # --- fixed-fiducial-model chi^2 distribution: covariance sanity ---
