"""
Masks and the North/South Galactic partition window.

Two ingredients:

1. `load_common_mask`  -> the Planck common temperature mask, downgraded to the
   working nside, binarized and apodized. Cached to disk.

2. `galactic_hemisphere_weight` -> a smooth partition-of-unity weight W(b) for
   the northern Galactic hemisphere, with south weight = 1 - W. Because
   W + (1 - W) = 1 *exactly*, the composite map  W*m_north + (1-W)*m_south
   has no amplitude artefact at the seam (unlike apodizing two complementary
   top-hats and adding them). The seam at b=0 sits under the masked Galactic
   plane, so the transition is essentially invisible in the analysis.

All maps are treated directly in Galactic HEALPix pixelization (the common mask
is Galactic), so no coordinate rotation is needed: b = 90deg - theta.
"""
...
def load_common_mask(cfg: RunConfig, verbose: bool = True) -> np.ndarray:
    """Load, downgrade, binarize and apodize the common mask (cached)."""
    cache = os.path.join(
        cfg.cache_dir,
    ...
    m = (m >= 0.5).astype(np.float64)          # binarize after downgrade
    if cfg.apod_deg > 0:
...
def galactic_hemisphere_weight(nside: int, blend_width_deg: float = 5.0,
    ...
    Split at b = 0. `blend_width_deg` is the tanh transition half-width; use 0
    for a sharp cut. Returns W in [0, 1]; the complementary hemisphere weight
    is 1 - W.
    """
...
def subtract_monopole(map_in: np.ndarray, mask: np.ndarray) -> np.ndarray:
    """Remove the mask-weighted mean (monopole) inside the observed region."""
...
def transfer_function(cfg: RunConfig) -> np.ndarray:
    ...
    The maps are generated with `pixwin=True` (and optionally beam-smoothed), so
    including the same pixel window in the theory removes the high-l suppression
    (~l >~ 2*nside) that otherwise biases the fit low. Real Planck maps carry the
    same pixel window, so this is also the physically correct model.
    """
