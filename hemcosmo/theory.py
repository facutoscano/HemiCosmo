...
All spectra are returned up to `lmax` (index = multipole, starting at l=0), in
muK^2. `dl=True` returns D_l = l(l+1)C_l/2pi; `dl=False` returns C_l with
l=0,1 set to zero.
"""
...
def cosmology_to_cls(cosmo: Cosmology, lmax: int,
    ...
    out[:n] = dltt[:n]          # this is D_l as CAMB returns it
...
def fitvec_to_cls(H0, ombh2, omch2, ns, Ase, tau, lmax,
                  lens_potential_accuracy: int = 1, dl: bool = False) -> np.ndarray:
    """Convenience: build a Cosmology from the fit basis and return its C_l."""
